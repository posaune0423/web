...
export const CanvasGrid: React.FC<CanvasGridProps> = ({
        ...
        if (maxCellSize) {
          const maxOffsetX = Math.max(
            ...
            maxCellSize * BASE_CELL_SIZE -
              (canvasRef.current?.width || 0) / newState.scale
          )
          const maxOffsetY = Math.max(
            0,
            maxCellSize * BASE_CELL_SIZE -
              (canvasRef.current?.height || 0) / newState.scale
          )
          return {
            ...newState,
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const { x, y } = convertClientPosToCanvasPos(
        canvasRef,
        e.clientX,
        e.clientY
      )
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (
        ...
        (Math.abs(dx) > SWIPE_THRESHOLD / 2 ||
          Math.abs(dy) > SWIPE_THRESHOLD / 2)
      ) {
      ...
      if (e.ctrlKey) {
        ...
        setLimitedGridState((prev) => {
          const newScale = Math.max(
            minZoom,
            Math.min(maxZoom, prev.scale * (1 + delta))
          )
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent<HTMLCanvasElement>) => {
      if (e.touches.length === 2) {
        ...
        const zoomFactor =
          currentDistance / gestureRef.current.lastPinchDistance

        setLimitedGridState((prev) => {
          const newScale = Math.max(
            minZoom,
            Math.min(maxZoom, prev.scale * zoomFactor)
          )
  ...
  const handleInertia = useCallback(() => {
    ...
    if (
      Math.abs(speedX) > INERTIA_STOP_THRESHOLD ||
      Math.abs(speedY) > INERTIA_STOP_THRESHOLD
    ) {
      setLimitedGridState((prev) => ({
        ...prev,
  ...
  const handleTouchEnd = useCallback(
    (e: React.TouchEvent<HTMLCanvasElement>) => {
      ...
      if (wasPinchGesture) {
        ...
        if (
          currentTime - gestureRef.current.lastPinchEndTime >
          PINCH_COOLDOWN
        ) {
